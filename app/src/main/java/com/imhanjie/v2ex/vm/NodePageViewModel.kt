...
class NodePageViewModel(private val nodeName: String, application: Application) : BasePageViewModel(application) {
    ...
    override suspend fun provideData(requestPage: Int): PageLiveData {
        return provideAppRepository().loadNodeTopics(nodeName, requestPage).let {
            ...
            PageLiveData(it.topics, it.currentPage != it.totalPage)
        }
    }
